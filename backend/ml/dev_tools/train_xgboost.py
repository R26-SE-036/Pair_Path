...
def load_dataset(path: str, demo_simulated: bool = False) -> pd.DataFrame:
    df = pd.read_csv(path)
    ...
    if demo_simulated:
        # Explicit demo path: simulated-ONLY. Mixing simulated rows into a
        # human dataset is never allowed — that's silent blending.
        if sources != {"simulated"}:
            sys.exit(
                f"[ABORT] --demo-simulated expects label_source == 'simulated' for every "
                f"row, found {sources}. Human and simulated data must never be mixed."
            )
        print("=" * 70)
        print("  DEMO MODE: training on SIMULATED data (pipeline demonstration).")
        print("  The resulting model card and every API response will be stamped")
        print("  demo_simulated. Metrics describe the generator, NOT students —")
        ...
        sys.exit(
            ...
            "For a pipeline demo on simulated data, pass --demo-simulated explicitly."
        )
...
def main():
    ...
    parser.add_argument(
        "--demo-simulated",
        action="store_true",
        help="Explicitly accept an all-simulated dataset for a PIPELINE DEMO. "
             "The model is stamped demo_simulated everywhere; its metrics are "
             "not performance claims.",
    )
    args = parser.parse_args()

    df = load_dataset(args.data, demo_simulated=args.demo_simulated)
    ...
    prefix = "demo_simulated" if args.demo_simulated else "xgb"
    version = f"{prefix}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M')}_{data_hash[:8]}"
    card = {
        "version": version,
        "trained_at": datetime.now(timezone.utc).isoformat(),
        "data_provenance": (
            "SIMULATED_DEMO — generated by dev_tools/generate_demo_sessions.py for "
            ...
            if args.demo_simulated else "human-annotated real sessions"
        ),
        ...
        "label_policy": "human annotation only (L1); 5-state taxonomy (L7)"
                        + (" — WAIVED for this demo model via --demo-simulated" if args.demo_simulated else ""),

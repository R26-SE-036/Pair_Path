...
export class SessionsService implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Whether the nudges changed what the model saw next, across this student's
   * own sessions. See nudge-effect.ts for what counts as "next", and for what
   * these numbers cannot tell anyone.
   *
   * Scoped to the student's own sessions like every other read here. The view
   * across all pairs is prisma/nudge-effect-report.ts, run by whoever holds
   * the database credentials.
   */
  async nudgeEffect(userId: string): Promise<NudgeEffect> {
    const sessions = await this.prisma.pairSession.findMany({
      where: { members: { some: { userId } }, interventions: { some: {} } },
      select: {
        interventions: {
    ...
    return measureNudges(
      sessions.flatMap((session) => session.interventions),
      sessions.flatMap((session) => session.predictions),
      ML_WINDOW_SECONDS,
    );
  }
}
